...
(defvar jest-test-mode-map
  (let ((map (make-sparse-keymap)))
    ;; (define-key map (kbd "SPC m t p")   'jest-test-run-all-tests)
    ;; (define-key map (kbd "SPC m t C-p") 'jest-test-run-all-tests)
    ;; (define-key map (kbd "SPC m t n")   'jest-test-run)
    ;; (define-key map (kbd "SPC m t C-n") 'jest-test-run)
    ;; (define-key map (kbd "SPC m t a")   'jest-test-rerun-test)
    ;; (define-key map (kbd "SPC m t C-a") 'jest-test-rerun-test)
    ;; (define-key map (kbd "SPC m t t")   'jest-test-run-at-point)
    ;; (define-key map (kbd "SPC m t C-t") 'jest-test-run-at-point)
    ;; (define-key map (kbd "SPC m t d n") 'jest-test-debug)
    ;; (define-key map (kbd "SPC m t d a") 'jest-test-debug-rerun-test)
    ;; (define-key map (kbd "SPC m t d t") 'jest-test-debug-run-at-point)
